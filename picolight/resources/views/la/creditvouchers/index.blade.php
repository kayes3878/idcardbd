...
@section("contentheader_title", "Creditvouchers")
@section("contentheader_description", "creditvouchers listing")
@section("section", "Creditvouchers")
@section("sub_section", "Listing")
@section("htmlheader_title", "Creditvouchers Listing")

@section("headerElems")
<button class="btn btn-success btn-sm pull-right" data-toggle="modal" data-target="#AddModal">Add Creditvoucher</button>
@endsection
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header modal-lg">
				...
				<h4 class="modal-title" id="myModalLabel">Add Creditvoucher</h4>
			</div>
			{!! Form::open(['action' => 'LA\CreditvouchersController@store', 'id' => 'creditvoucher-add-form']) !!}
...
$(function () {
	$("#example1").DataTable({
		processing: true,
        serverSide: true,
        ajax: "{{ url(config('laraadmin.adminRoute') . '/creditvoucher_dt_ajax') }}",
		language: {
			lengthMenu: "_MENU_",
			search: "_INPUT_",
	...
	$("#creditvoucher-add-form").validate({
...
if($.isNumeric(debit_amt))
{
	...
	"<td>"+account_title+"</td>"+
	"<td id='age_row"+table_len+"'><input type='text' class='text-centern detailamnt' style='border:none;' name='debit_amt["+table_len+"]' value='"+debit_amt+"' readonly></td>"+
	"<td>"+	
